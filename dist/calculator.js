...
exports.calculateMaxPriceForDti = calculateMaxPriceForDti;
exports.calculateAffordability = calculateAffordability;
exports.calculateScoreAtPrice = calculateScoreAtPrice;
exports.calculateScoreFromValues = calculateScoreFromValues;
exports.calculateScore = calculateScore;
// Credit score range to approximate score mapping
function getCreditScore(range) {
    if (!range)
        return null;
    const mapping = {
        "below-580": 550,
...
function calculateCreditPoints(creditScore) {
    if (!creditScore)
        return 15; // Unknown - middle estimate
    if (creditScore >= 740)
        return 30;
    if (creditScore >= 720)
        return 27;
...
function getIncomeAmount(range) {
    if (!range)
        return 60000; // Default assumption
    const mapping = {
        "under-40k": 35000,
        "40k-60k": 50000,
        "60k-80k": 70000,
...
function getPriceAmount(range) {
    if (!range)
        return 500000; // Default assumption (Utah avg ~$589k)
    const mapping = {
        "under-400k": 350000,
        "400k-500k": 450000,
        "500k-600k": 550000,
...
function getDownPaymentAmount(range) {
    if (!range)
        return 20000; // Default assumption
    const mapping = {
        "under-10k": 5000,
        "10k-25k": 17500,
        "25k-50k": 37500,
...
function getMonthlyDebtAmount(range) {
    if (!range)
        return 0; // Default to no debt if not specified
    const mapping = {
        none: 0,
        "under-250": 125,
        "250-500": 375,
...
function estimateMonthlyPayment(price, downPaymentPercent = 0.035) {
...
function calculateDtiPoints(monthlyIncome, targetPrice, monthlyDebts) {
...
function calculateDownPaymentPoints(saved, targetPrice, veteranStatus) {
...
function calculateEmploymentPoints() {
...
function calculateReservesPoints(saved, targetPrice) {
...
function calculateBonusPoints(firstTimeBuyer, veteranStatus) {
    let bonus = 0;
    // Veteran status
    if (veteranStatus &&
        ["active", "veteran", "guard-reserve", "spouse"].includes(veteranStatus)) {
...
// Match programs
function matchPrograms(creditScore, firstTimeBuyer, veteranStatus, income) {
    const programs = [];
...
// Identify gaps
function identifyGaps(creditScore, creditPoints, downPaymentPoints, dtiPoints, saved, targetPrice) {
    const gaps = [];
    // Credit gap
    if (creditPoints < 20) {
        gaps.push({
        ...
        switch (gap.factor) {
            case "credit":
                return {
                    ...
                    description: "Pay down credit cards to below 30% of limits, avoid new credit applications, and dispute any errors on your report.",
            ...
            case "down_payment":
                return {
                    ...
                    description: "Set up automatic transfers to savings. Look into down payment assistance programs you may qualify for.",
                    impact: `+${gap.potentialGain} points to your Home Ready Score`,
                };
            case "dti":
                return {
                    ...
                    description: "Focus on paying off high-interest debt or consider a lower price point to improve your DTI ratio.",
                    impact: `+${gap.potentialGain} points to your Home Ready Score`,
                };
            default:
                return {
...
function checkDPAEligibility(creditScore, annualIncome, targetPrice, firstTimeBuyer, veteranStatus) {
    ...
    const firstHomeEligible = firstTimeBuyer === true &&
    ...
    const homeAgainEligible = score >= PROGRAM_LIMITS.homeAgain.minCreditScore &&
...
function calculateDebtReductionForDti(monthlyIncome, currentDebts, housingPayment, targetDti) {
...
function calculateIncomeForDti(currentIncome, monthlyDebts, housingPayment, targetDti) {
...
function calculateCurrentDti(monthlyIncome, monthlyDebts, housingPayment) {
    return Math.round(((housingPayment + monthlyDebts) / monthlyIncome) * 100);
}
// Detect the primary blocker preventing home readiness
function detectPrimaryBlocker(creditScore, creditPoints, dtiPoints, downPaymentPoints, monthlyIncome, monthlyDebts, targetPrice, saved, veteranStatus, annualIncome, firstTimeBuyer) {
    // Check DPA eligibility upfront
    const dpaEligibility = checkDPAEligibility(creditScore, annualIncome, targetPrice, firstTimeBuyer, veteranStatus);
...
function calculateSweetSpot(monthlyIncome, monthlyDebts, saved, targetPrice, input) {
...
function calculatePathToGoal(monthlyIncome, monthlyDebts, saved, targetPrice, sweetSpot, currentScore) {
...
function calculateScoreInternal(input) {
    ...
    const total = Math.max(0, Math.min(100, creditPoints + dtiPoints + downPaymentPoints + employmentPoints + reservesPoints + bonusPoints));
    return {
        total,
...
function calculateMaxPriceForDti(monthlyIncome, monthlyDebts, targetDti, downPaymentPercent = 0.035) {
...
function calculateAffordability(annualIncome, monthlyDebtsRange, targetPriceRange) {
...
function calculateScoreAtPrice(input, overridePrice) {
...
function getPriceRangeFromAmount(price) {
    if (price < 400000)
        return "under-400k";
    if (price < 500000)
        return "400k-500k";
...
function getDownPaymentRange(amount) {
    if (amount >= 260000)
        return "260k-plus";
    if (amount >= 220000)
        return "220k-260k";
...
function getMonthlyDebtRange(debt) {
    if (debt >= 3000)
        return "3000-plus";
    if (debt >= 2500)
        return "2500-3000";
 ...
 */
function calculateScoreFromValues(input) {
...
function calculateScore(input) {
    ...
    // Match programs
    const programDetails = matchPrograms(creditScore, input.firstTimeBuyer, input.veteranStatus, income);
    const programs = programDetails
        .filter((p) => p.eligible)
        .map((p) => p.name);
    // Identify gaps
    const gaps = identifyGaps(creditScore, creditPoints, downPaymentPoints, dtiPoints, saved, targetPrice);
    // Generate recommendations
    ...
    const primaryBlocker = detectPrimaryBlocker(creditScore, creditPoints, dtiPoints, downPaymentPoints, monthlyIncome, monthlyDebts, targetPrice, saved, input.veteranStatus, income, input.firstTimeBuyer);
    const sweetSpot = calculateSweetSpot(monthlyIncome, monthlyDebts, saved, targetPrice, input);
    const pathToGoal = calculatePathToGoal(monthlyIncome, monthlyDebts, saved, targetPrice, sweetSpot, total);
    return {
        total,
        status,
        timeline,
